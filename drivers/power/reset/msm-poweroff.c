...
/*req SHK-335,luohao,20231120,add,end*/
	...
	if (cmd != NULL) {
		...
		// Add by zhixue.chang for reboot to red screen if check modem is not signed 20231018 begin
		if (strncmp(cmd, "modem_unsigned", sizeof("modem_unsigned")) == 0) {
			reason = PON_RESTART_REASON_MODEM_UNSIGNED;
		}
		// Add by zhixue.chang for reboot to red screen if check modem is not signed 20231018 end
