...
#define pr_fmt(fmt)	"[cne-charge][pdc_intf] %s: " fmt, __func__
...
int cne_pdc_set_mivr(struct charger_manager *info, int uV)
{
	int ret = 0;
...
void cne_pdc_check_cable_impedance(struct charger_manager *pinfo)
{
	int ret = 0;
	int vchr1, vchr2, cable_imp;
	unsigned int aicr_value;
	bool mivr_state = false;
	ktime_t k_time[2], k_diff;
	...
	k_time[0] = ktime_get_boottime();

	/* Set ichg = 2500mA, set MIVR */
	charger_dev_set_charging_current(pinfo->chg1_dev, 2500000);
	mdelay(240);
	ret = cne_pdc_set_mivr(pinfo, pinfo->data.min_charger_voltage);
	if (ret < 0)
		chr_err("%s: failed, ret = %d\n", __func__, ret);

	// get_monotonic_boottime(&ptime[1]);
	k_time[1] = ktime_get_boottime();
	k_diff = ktime_sub(k_time[1], k_time[0]);
	diff = ktime_to_timespec64(k_diff);
...
static bool cne_is_pdc_ready(struct charger_manager *info)
{
...
{
	if (cne_is_pdc_ready(info) == false)
		return false;
...
{
	int charging_current = info->data.pd_charger_input_current / 1000;
	int vbat = battery_get_bat_voltage(info);
	...
	if (info->pdc.pdc_max_watt_setting != -1)
		...
		if ((info->chg1_data.user_charging_current_limit != -1) &&
			((info->chg1_data.user_charging_current_limit / 1000) < charging_current))
			charging_current =
			info->chg1_data.user_charging_current_limit / 1000;
...
int cne_pdc_get_idx(struct charger_manager *info, int selected_idx,
	int *boost_idx, int *buck_idx)
{
	struct cne_pdc *pd = &info->pdc;
...
int cne_pdc_setup(struct charger_manager *info, int idx)
{
	...
	if (pd->pd_idx != idx || force_update) {
		...
		cne_pdc_set_mivr(info, info->data.min_charger_voltage);
		...
		ret = adapter_dev_set_cap(info->pd_adapter, CNE_PD,
			pd->cap.max_mv[idx], pd->cap.ma[idx]);

		if (ret == CNE_ADAPTER_OK) {
			...
			cne_pdc_set_mivr(info, mivr * 1000);
		} else {
			if (info->data.parallel_vbus &&
				(oldmA * 2 > pd->cap.ma[idx])) {
				charger_dev_set_input_current(info->chg1_dev,
			...
			cne_pdc_set_mivr(info, oldmivr);
		}

		cne_pdc_get_idx(info, idx, &pd->pd_boost_idx, &pd->pd_buck_idx);
	}
...
void cne_pdc_get_cap_max_watt(struct charger_manager *info)
{
	struct cne_pdc *pd = &info->pdc;
	struct adapter_power_cap *cap;
	int i = 0;
...
void cne_pdc_get_reset_idx(struct charger_manager *info)
{
	struct cne_pdc *pd = &info->pdc;
	struct adapter_power_cap *cap;
	int i = 0;
...
void cne_pdc_reset(struct charger_manager *info)
{
	...
	cne_pdc_init_table(info);
	cne_pdc_get_reset_idx(info);
	cne_pdc_setup(info, pd->pd_reset_idx);
}

int cne_pdc_get_setting(struct charger_manager *info, int *newvbus, int *newcur,
			int *newidx)
{
	int ret = 0;
	...
	if (info->data.parallel_vbus) {
		...
		if (ret < 0) {
			ibat = battery_get_bat_current(info);
		...
		if (ibat < 0 || chg2_ibat < 0)
			...
			chg2_watt = chg2_ibat / 1000 * battery_get_bat_voltage(info)
...
void cne_pdc_init_table(struct charger_manager *info)
{
	...
	if (cne_is_pdc_ready(info))
		adapter_dev_get_cap(info->pd_adapter, CNE_PD, &pd->cap);
	else
