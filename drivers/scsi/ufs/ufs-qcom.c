...
/* req SHK-5775,zhanghaozhuo,20231121,add,begin */
static bool ufs_get_value_of_cmdline(void)
  {
  	struct device_node *np;
  	...
  	else if (strnstr(bootparams, "ontimboot.powpstate=Enable",strlen(bootparams)))
  		return true;
  
  	return false;
  
  }
/* req SHK-5775,zhanghaozhuo,20231121,add,end */
/**
...
static int ufs_qcom_probe(struct platform_device *pdev)
{
	...
	/* req SHK-5775 ,SHK-6248 ,zhanghaozhuo,20231130,add,begin */
             hba = platform_get_drvdata(pdev);
             if(ufs_get_value_of_cmdline()&&hba!=NULL) {
               if(!ufshcd_query_flag_retry(hba, UPIU_QUERY_OPCODE_SET_FLAG,QUERY_FLAG_IDN_PWR_ON_WPE, 0, NULL)) {
                     pr_err("The USERDEBUG and ENG enable partition write protection successfully");
               }
             }
	}
        /* req SHK-5775 ,SHK-6248 ,zhanghaozhuo,20231130,add,end */
