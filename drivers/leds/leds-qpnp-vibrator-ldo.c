...
struct vib_ldo_chip {
	...
	//req SEAH-818 xuzhixian 20231010 begin
	u32                     en_gpio;
	u32                     en_gpio_flags;
	//req SEAH-818 xuzhixian 20231010 end
};

#if 0 //req SEAH-818 xuzhixian 20231010
...
#endif //req SEAH-818 xuzhixian 20231010
...
static int qpnp_vibrator_play_on(struct vib_ldo_chip *chip)
{
	...
	mdelay(25); //add for vibrator so weak
	if (ret)
...
static enum hrtimer_restart vib_overdrive_timer(struct hrtimer *timer)
{
	...
	//				     overdrive_timer); //req SEAH-818 xuzhixian
	//schedule_work(&chip->overdrive_work); //req SEAH-818 xuzhixian
			...
			enum led_brightness level)
{
	...
	if (chip->state) {
		...
		if (!chip->disable_overdrive) {
			...
			//cancel_work_sync(&chip->overdrive_work); //req SEAH-818 xuzhixian
		}
		ret = qpnp_vibrator_play_off(chip); //req SEAH-818 xuzhixian 20231010
	}
...
static int qpnp_vibrator_ldo_suspend(struct device *dev)
{
	...
	qpnp_vibrator_play_off(chip); //req SEAH-818 xuzhixian 20231010
	mutex_unlock(&chip->lock);
