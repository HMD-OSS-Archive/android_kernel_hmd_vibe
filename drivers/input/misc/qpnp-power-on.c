...
static int qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	if (cfg->pon_type == PON_KPDPWR) {
	    ...
	    printk(KERN_ERR"Report pwrkey %s event,key_code:%d\n", key_status ? "press" : "release",
	                                                            cfg->key_code);
        }
...
// ning.wei@hmd++ start
static ssize_t volume_minus_trigger_ramdump_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = dev_get_drvdata(dev);

	qpnp_pon_write(pon, QPNP_PON_RESIN_S1_TIMER(pon), 0);
	qpnp_pon_write(pon, QPNP_PON_RESIN_S2_TIMER(pon), 0);
	qpnp_pon_write(pon, QPNP_PON_RESIN_S2_CNTL(pon), 1);
	qpnp_pon_write(pon, QPNP_PON_RESIN_S2_CNTL2(pon), 0x80);

	return scnprintf(buf, PAGE_SIZE, "You can press volume minus key to trigger ramdump now !\n");
}
static DEVICE_ATTR_RO(volume_minus_trigger_ramdump);
// ning.wei@hmd++ end
...
static int qpnp_pon_probe(struct platform_device *pdev)
{
	...
	// ning.wei@hmd++
	rc = device_create_file(dev, &dev_attr_volume_minus_trigger_ramdump);
	if (rc) {
		dev_err(dev, "sysfs vol- trigger dump file creation failed, rc=%d\n",
			rc);
	}
	// ning.wei@hmd++
